...
function scene:createScene( event )
  ...
  local function loadStars (level)
    if (gameSettings[3][level][2] ~=nil) then
      if (gameSettings[3][level][2] > globals.stars[3][level]) then
        globals.stars[3][level] = gameSettings[3][level][2]
      end
    end
  end

  -- sets all the level buttons
  for i=1, table.maxn( levelButtons3 ) do
    ...
    if (globals.stars[3][i]~= nil) then
      if (globals.stars[2][i] == 0) then
       ...
       if (globals.stars[3][i] >= 1) then
        ...
        levelButtons3[i] = nil
        levels_1[i].alpha = 1
        levelButtons3[i] = levels_1[i]
        levelButtons3[i].x = 55*i
      end
      if (globals.stars[3][i] >= 2) then
        --sets the button to have 2 stars
        levelButtons3[i]:removeSelf( )
        levelButtons3[i] = nil
        levels_2[i].alpha = 1
        levelButtons3[i] = levels_2[i]
        levelButtons3[i].x = 55*i
      end
      if (globals.stars[3][i] >= 3) then
        --sets the button to have 3 stars
        levelButtons3[i]:removeSelf( )
        levelButtons3[i] = nil
        levels_3[i].alpha = 1
        levelButtons3[i] = levels_3[i]
        levelButtons3[i].x = 55*i
      end
    end
    scrollView:insert(  levelButtons3[i] )
  end
...
--for the stars to show objectives and sets to alpha until they are called
  ...
  local function onTapStar1 ( event )
   ...
   local function onTapLevel1 ( event )
        levelSelect = 1
        selectedLevel = Level.load(3, levelSelect)
        levelTitle.text = selectedLevel.levelTitle;
        highScore.text = "High Score: " .. gameSettings[3][levelSelect][1];
  ...
  local function onTapStartLevel (event)
    if( levelSelect < 10 or ( levelSelect == 10 --[[  and worlds[1].levelsUnlocked[10] ]]) ) then
      storyboard.removeScene( scene )
      storyboard.gotoScene( "scenes.scene_ingame", { effect = "fade", time = 250, params = {level = levelSelect, world = 3}})
    end
  end
  
  --adds all the event listeners to the level button
  levelButtons3[1]:addEventListener("tap", onTapLevel1)
  levelButtons3[2]:addEventListener("tap", onTapLevel2)
  levelButtons3[3]:addEventListener("tap", onTapLevel3)
  levelButtons3[4]:addEventListener("tap", onTapLevel4)
  levelButtons3[5]:addEventListener("tap", onTapLevel5)
  levelButtons3[6]:addEventListener("tap", onTapLevel6)
  levelButtons3[7]:addEventListener("tap", onTapLevel7)
  levelButtons3[8]:addEventListener("tap", onTapLevel8)
  levelButtons3[9]:addEventListener("tap", onTapLevel9)
  levelButtons3[10]:addEventListener("tap", onTapLevel10)
